          ...
          <dl>
            ...
            <dd>
              ...
              <m-select prop="action" label="Security Group" labelWidth="100px" validateEvent
                @on-change="handleGroupSelect" v-model="action" @validate="validate('action')" required
                :show-error="rules['action'].error" :error-msg="rules['action'].message"
...
export default {
  name: "CreateFireWallGroupRuleModal",
  mixins: [FirewallHttp],
  components: {
    Dialog,
  },
  ...
  methods: {
    ...
    async __init__() {
      ...
      if (this.isCreate) {
        ...
        if (_this.param && String(_this.param.pos)) {
          _this
            .queryFireWallGroupRuleById(_this.group, _this.param.pos)
            .then(() => {
    ...
    confirm() {
      ...
      if (this.isCreate || this.modalType === "copy") {
        this.createFireWallGroupRule(_this.group, param)
          .then((res) => {
            this.close();
          })
          .catch((res) => {
        ...
        this.updateFireWallGroupRule(this.param.pos, param, _this.group)
          .then((res) => {
            this.close();
          })
          .catch((res) => {
