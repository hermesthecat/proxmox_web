<template>
  <page-template>
    <div slot="toolbar-left">
      <m-button type="primary" @on-click="showModal('create')" icon="el-icon-plus">Create</m-button>
    ...
    <div slot="page-content">
      <el-table :data="db.fireWallGroupList" ref="dataTable" :row-key="setRowKeys" :expand-row-keys="expands"
        @selection-change="handleSelect" @expand-change="expandChange">
        <el-table-column type="expand">
          <template slot-scope="parentSlot">
            <page-template>
              <div slot="toolbar-left">
                <m-button type="primary" @on-click="showRuleModal('create')" icon="el-icon-plus">Add</m-button>
                <m-button type="warning" @on-click="showRuleModal('copy')" icon="el-icon-copy-document"
                  :disabled="selectedRuleList && selectedRuleList.length !== 1">Copy</m-button>
                <m-button type="danger" v-confirm="{
                  msg: 'Are you sure you want to delete the selected items?',
                  ok: () => handleGroupRuleDelete(),
                  icon: 'icon-question',
                }" icon="el-icon-delete" :disabled="selectedRuleList.length <= 0">Delete</m-button>
                <m-button type="info" icon="el-icon-edit" :disabled="selectedRuleList.length !== 1"
                  @on-click="showRuleModal('edit')">Edit</m-button>
              </div>
              <div slot="page-content">
                <el-table :data="db.fireWallRuleList" @selection-change="handleRuleSelect">
                  ...
                  <el-table-column label="Enable" prop="enable">
                    <template slot-scope="scope">
                      <table-info-state :content="scope.row.enable && scope.row.enable === 1
                          ? 'Yes'
                          : 'No'
      ...
      <create-fire-wall-group-rule-modal :title="ruleTitle" :isCreate="isRuleCreate" :param="ruleParam"
        :visible="ruleVisible" v-if="ruleVisible" :modal-type="ruleType" :group="group" @close="
          ruleVisible = false;
        queryFireRuleList(group);
        "></create-fire-wall-group-rule-modal>
      <create-fire-wall-group-modal :title="title" :isCreate="isCreate" :param="param" :visible="visible" v-if="visible"
