    ...
    async __init__() {
      ...
      await _this
        ...
        .then((res) => {
          ...
          if (
            _this.db.cephObj &&
            _this.db.cephObj.health &&
            _this.db.cephObj.health.checks
          ) {
            let healthList = Object.keys(_this.db.cephObj.health.checks).map(
              (key) => {
                if (
                  !_this.clsMap.hasOwnProperty(
                    _this.db.cephObj.health.checks[key].severity
                  )
                ) {
                  _this.clsMap[_this.db.cephObj.health.checks[key].severity] =
                    _this.db.cephObj.health.checks[key].severity
                      .replace(/HEALTH_/, "")
                      .toLowerCase();
                }
                ...
                if (key === "OSD_DOWN") {
                  found =
                    _this.db.cephObj.health.checks[key].summary.message.match(
                      downinregex
                    );
                  if (found !== null) {
          ...
          if (
            _this.db.cephObj &&
            _this.db.cephObj.osdmap &&
            _this.db.cephObj.osdmap.osdmap
          ) {
          ...
          if (_this.db.cephObj && _this.db.cephObj.pgmap) {
            _this.statecategories.forEach((cat, index) => {
              _this.$set(_this.$data.statecategories, index, {
                ...cat,
                count: 0,
                states: [],
                desc: "",
          ...
          if (
            _this.db.cephObj &&
            _this.db.cephObj.pgmap &&
            _this.db.cephObj.pgmap.bytes_used &&
            _this.db.cephObj.pgmap.bytes_total
          ) {
            let text = stringFormat(
              "{0} of {1}",
              byteToSize(_this.db.cephObj.pgmap.bytes_used),
              byteToSize(_this.db.cephObj.pgmap.bytes_total)
            );
            _this.usage.used =
              _this.db.cephObj.pgmap.bytes_used /
        ...
        if (_this.db.cephMetaDataObj && _this.db.cephMetaDataObj.mon) {
          ...
          for (let mon of Object.keys(this.mon)) {
            let tmp = mon.split("@"),
              ...
              host = tmp[1];
            if (
              this.db.cephObj.quorum_names &&
              this.db.cephObj.quorum_names.indexOf(name) > -1
            ) {
            ...
            if (_this.mgrmessages && _this.mgrmessages[name] === "active") {
              if (!_this.mgr[mgr].statuses)
                ...
                _this.$set(
                  _this.mgr[mgr],
                  "statuses",
                  _this.mgr[mgr].statuses.concat("active")
                );
              ...
              if (!_this.mgr[mgr].statuses)
                ...
                _this.$set(
                  _this.mgr[mgr],
                  "statuses",
                  _this.mgr.statuses.concat("standby")
                );
          ...
          for (let mds of Object.keys(this.mds)) {
            ...
            if (_this.mgrmessages && _this.mgrmessages[name]) {
              if (!_this.mds[mds].statuses)
                _this.$set(_this.mds[mds], "statuses", [
                  _this.mgrmessages[name],
                ]);
              else
                _this.$set(
                  _this.mds[mds],
                  "statuses",
                  _this.mds[mds].statuses.concat(_this.mgrmessages[name])
                );
              ...
              if (!_this.mds[mds].statuses)
                ...
                _this.$set(
                  _this.mds[mds],
                  "statuses",
                  _this.mds[mds].statuses.concat("standby")
                );
            }
          }
        }
