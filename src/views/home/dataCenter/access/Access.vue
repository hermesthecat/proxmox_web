<template>
  <page-template>
    ...
    <div slot="page-content">
      <el-table :data="db.accessList" ref="dataTable" @selection-change="handleSelect">
        ...
        <el-table-column label="Propagate" prop="propagate">
          <template slot-scope="scope">
            <table-info-state :content="scope.row.propagate && scope.row.propagate === 1 ? 'Yes' : 'No'
              " :state="scope.row.propagate && scope.row.propagate === 1
                  ? 'actived'
                  : 'unActived'
                "></table-info-state>
          </template>
