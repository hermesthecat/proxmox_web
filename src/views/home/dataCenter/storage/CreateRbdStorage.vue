    ...
    <div class="m-form__section">
      <dl>
        ...
        <dd>
          ...
          <m-select labelWidth="100px" prop="pool" @on-change="(value) => (pool = value)" validateEvent
            @validate="validate" required :show-error="rules.pool.error" :error-msg="rules.pool.message" v-model="pool"
            :disabled="!isCreate" label="Resource Pool">
            <m-option v-for="item in cephPoolsList" :key="item.pool_name" :label="item.pool_name"
              :value="item.pool_name"></m-option>
          </m-select>
          <m-input type="text" prop="monhost" label="Monitor(s)" labelWidth="100px" validateEvent @validate="validate"
            required :disabled="pveceph" :show-error="rules.monhost.error" :error-msg="rules.monhost.message"
            v-model="monhost" placeholder="Please enter monitor" />
          <m-checkbox label="Enable" v-model="disable" labelWidth="100px"></m-checkbox>
          <m-input type="text" prop="username" label="Username" labelWidth="100px" v-model="username"
            :disabled="!isCreate || pveceph" placeholder="Please enter username" />
          <m-select type="multiple" labelWidth="100px" @on-change="handleSelect" validateEvent @validate="validate"
            required prop="content" v-model="content" :show-error="rules.content.error"
          ...
          <m-checkbox label="KRBD" v-model="krbd" labelWidth="100px"></m-checkbox>
          <m-checkbox label="" v-model="pveceph" :disabled="!pvecephPossible" @change="(value) => {
              if (value) {
                monhost = quickSort(
            ...
            " labelWidth="100px">Use Proxmox VE managed hyper-converged ceph pool</m-checkbox>
        </dd>
      </dl>
      <dl>
        <dt>Nodes</dt>
