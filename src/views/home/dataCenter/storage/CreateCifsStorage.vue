      ...
      <dl>
        ...
        <dd>
          <m-input type="text" prop="storage" label="ID" labelWidth="100px" validateEvent @validate="validate" required
            :show-error="rules.storage.error" :error-msg="rules.storage.message" v-model="storage"
            placeholder="Please enter ID" :disabled="!isCreate" />
          ...
          <m-input type="text" prop="server" label="Server" labelWidth="100px" validateEvent @validate="validate"
            ...
            placeholder="Please enter server" :disabled="!isCreate" />
          <m-input type="text" prop="username" label="Guest User" labelWidth="100px" :disabled="!isCreate" validateEvent
            @validate="validate" required :show-error="rules.username.error" :error-msg="rules.username.message"
            v-model="username" placeholder="Please enter guest user" />
          <m-input v-if="isCreate" type="password" prop="password" label="Password" labelWidth="100px" validateEvent
            @validate="validate" required :show-error="rules.password.error" :error-msg="rules.password.message"
            :disabled="!username || !isCreate" v-model="password" placeholder="Please enter password" />
          <m-input v-if="!isCreate" type="password" prop="password" label="Password" labelWidth="100px"
            :disabled="!username || !isCreate" value="******" />
          <m-select labelWidth="100px" @on-change="handleCifsSelect" validateEvent @validate="validate" prop="share"
            v-model="share" :readonly="false" :disabled="!isCreate" required @visible-change="handleExportReq"
            :show-error="rules.share.error" :error-msg="rules.share.message" label="Share">
            <m-option v-for="item in db.cifsList" :key="item.share" :label="item.share" :value="item.share"></m-option>
          </m-select>
          ...
          <m-input type="text" prop="domain" label="Domain" labelWidth="100px" validateEvent @validate="validate"
            required :show-error="rules.domain.error" :error-msg="rules.domain.message" min="0" v-model="domain"
            :disabled="!isCreate" placeholder="Please enter domain" />
        </dd>
      </dl>
      <dl v-if="isAdvice">
        <dt>Advanced</dt>
        <dd>
          <m-select v-model="option" :read-only="true" @on-change="(value) => (option = value)" label="Version">
