...
export default {
  ...
  methods: {
    ...
    queryAcmeAccountById(id) {
      return this.$http
        .get(`json/cluster/acme/account/${id}`, {
          _dc: new Date().getTime(),
        })
        .then((res) => {
          if (res.data) {
    ...
    queryDNSApiList() {
      return this.$http
        .get("json/cluster/acme/challenge-schema")
        .then((res) => {
          if (res.data) {
            this.updateTable({
    ...
    createPlugin(param) {
      ...
      return this.$http
        .post("json/cluster/acme/plugins/", param, {
          headers: {
    ...
    queryAcmePluginById(id) {
      return this.$http
        .get(`json/cluster/acme/plugins/${id}`, {
          _dc: new Date().getTime(),
        })
        .then((res) => {
          if (res.data) {
    ...
    updateDnsApiPlugin(id, param) {
      ...
      return this.$http
        .put("json/cluster/acme/plugins/" + id, param, {
          headers: {
            "content-type": "application/x-www-form-urlencoded; charst=utf8",
          },
        })
