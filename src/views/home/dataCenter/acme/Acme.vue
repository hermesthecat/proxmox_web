        ...
        <div class="card-item">
          <page-template>
            ...
            <div slot="page-content">
              <el-table :data="db.acmeAccountList" ref="dataTable" @selection-change="handleSelect">
    ...
    <overview-card>
      ...
      <div slot="content" class="card-content" v-if="showResource">
        <div class="card-item">
          <page-template>
            <div slot="toolbar-left">
              <m-button type="primary" @on-click="showPlginModal('create')" icon="el-icon-plus">Add</m-button>
              <m-button type="info" @on-click="showPlginModal('edit')" icon="el-icon-edit"
                :disabled="selectedPluginList.length !== 1">Edit</m-button>
              <m-button type="danger" v-confirm="{
                msg: 'Are you sure you want to delete the selected items?',
                ok: () => handlePluginDelete(),
                icon: 'icon-question',
              }" icon="el-icon-delete" :disabled="selectedPluginList.length <= 0">Delete</m-button>
            </div>
            <div slot="page-content">
              <el-table :data="acmePluginList" ref="dataTable" @selection-change="handlePluginSelect">
                <el-table-column type="selection" width="55"></el-table-column>
          ...
          <create-acme-account-modal :title="title" :isCreate="isCreate" :param="param" :visible="visible"
            v-if="visible" :modal-type="type" @close="
              visible = false;
            queryAcmeAccountList();
            "></create-acme-account-modal>
          <create-acme-plugin-modal :title="pluginTitle" :isCreate="pluginIsCreate" :param="pluginParam"
            :visible="pluginVisible" v-if="pluginVisible" :modal-type="pluginType" @close="
              pluginVisible = false;
            __init__();
