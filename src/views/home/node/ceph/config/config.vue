  ...
  <div class="ceph-config">
    ...
    <div class="ceph-config-left">
      <overview-card>
        ...
        <div slot="content" class="card-content">
          <ace-editor v-model="crush" ref="left" :read-only="true"></ace-editor>
        </div>
      </overview-card>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    __init__() {
      this.queryConfig("config", { _dc: new Date().getTime() }).then((res) => {
        this.config = res;
      });
      this.queryConfig("crush", { _dc: new Date().getTime() }).then((res) => {
        this.crush = res;
      });
      this.queryConfig("configdb").then((res) => {
        this.configdb = res;
      });
    },
  },
