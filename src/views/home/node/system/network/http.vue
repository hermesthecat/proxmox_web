...
export default {
  ...
  methods: {
    queryNetWorkList(param) {
      return this.$http
        .get(`json/nodes/${this.node}/network`, param)
        .then((res) => {
          if (res.data) {
            this.updateTable({
    ...
    createNetWork(param) {
      ...
      return this.$http
        .post(`json/nodes/${this.node}/network`, param, {
          headers: {
    ...
    resume() {
      ...
      return this.$http
        .del(`json/nodes/${this.node}/network`)
        .then((res) => {
          this.incEventSuccess(event);
    ...
    updateNetWork(iface, param) {
      ...
      return this.$http
        .put(`json/nodes/${this.node}/network/${iface}`, param, {
          headers: {
            "content-type": "application/x-www-form-urlencoded; charset=utf8",
          },
        })
    ...
    deleteNetWork() {
      ...
      this.selectedList.forEach((it) => {
        p = ((it) => {
          this.$http
            .del(`json/nodes/${this.node}/network/${it.iface}`)
            .then((res) => {
              this.incEventSuccess(event);
            })
            .catch((res) => {
