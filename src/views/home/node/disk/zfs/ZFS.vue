<template>
  <page-template>
    <div slot="toolbar-left">
      ...
      <m-button type="primary" @on-click="showModal('create')" icon="el-icon-plus">Create: ZFS</m-button>
      <m-button type="primary" @on-click="showModal('detail')" :disabled="selectedList.length !== 1"
        icon="el-icon-plus">Details</m-button>
    </div>
    <div slot="page-content">
      <el-table :data="db.nodeDiskZfsList" ref="dataTable" @selection-change="handleSelect" v-loading="loading">
        ...
        <el-table-column label="Health" prop="health">
          <template slot-scope="scope">
            <table-info-state :content="scope.row.health"
              :state="scope.row.health && scope.row.health.toLowerCase()"></table-info-state>
          </template>
        </el-table-column>
      </el-table>
      <create-zfs-modal :title="title" :visible="visible" v-if="visible" :modal-type="type" :param="param" @close="
        visible = false;
      __init__();
      "></create-zfs-modal>
    </div>
