...
export default {
  ...
  methods: {
    ...
    async __init__() {
      ...
      _this.queryResource().then((res) => {
        this.vmList = res.filter((it) => {
          if (_this.param.operate === "stopall")
            return (
              ["lxc", "qemu"].includes(it.type) &&
              it.node === _this.param.node &&
              it.status === "running"
            );
          ...
          if (_this.param.operate === "migrateall")
            return (
              ["lxc", "qemu"].includes(it.type) && it.node === _this.param.node
            );
        });
      ...
      if (_this.param.operate === "migrateall") {
        _this.queryNodeList().then((res) => {
          let nodeList = _this.db.nodeList.filter(
            (item) => item.node !== _this.param.node
          );
      ...
      switch (this.param.operate) {
        ...
        case "migrateall":
          ...
          Object.assign(param, {
            ...
            "with-local-disks": this.withLocalDisks ? 1 : 0,
          });
          this.betach(this.param.operate, param);
          break;
      }
    ...
    betach(operate, param) {
      let _this = this;
      _this
        .beatchOperate(operate, _this.param.node, param)
        .then((res) => {
          _this.showLog = true;
