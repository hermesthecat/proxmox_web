...
export default {
  ...
  methods: {
    ...
    queryStorage(param) {
      return this.$http
        .get(`json/nodes/localhost/storage`, param)
        .then((res) => {
          if (res.data) {
            this.updateTable({
    ...
    queryResource() {
      return this.$http.get(`json/cluster/resources`).then((res) => {
        if (res.data) {
          this.updateTable({
            tableName: "resources",
            list: res.data,
    ...
    updatePool(param) {
      ...
      return this.$http
        .put(`/json/pools/${this.pools.pool}`, param, {
          headers: {
    ...
    deleteResourceToPool() {
      let event = this.createEvent("action.pool.delete"),
        p,
        tasks = [];
      this.selectedList.forEach((item) => {
        ((item) => {
          p = this.$http
            .put(
              `/json/pools/${this.pools.pool}`,
              {
                delete: 1,
                [item.type === "storage" ? "storage" : "vms"]:
                  item.type === "storage" ? item.storage : item.vmid,
              },
              {
                headers: {
                  "content-type":
