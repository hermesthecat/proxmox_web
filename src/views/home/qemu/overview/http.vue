...
export default {
  ...
  methods: {
      ...
      this.$http
        ...
        .then((res) => {
          ...
          this.memory = Object.assign({}, this.memory, {
            value: [
              res.data.map((it) => (it.maxmem ? it.maxmem : 0)),
              res.data.map((it) => (it.mem ? it.mem : 0)),
            ],
            color: this.colors,
          ...
          this.network = Object.assign({}, this.network, {
            value: [
              res.data.map((it) => (it.netin ? it.netin : 0)),
              res.data.map((it) => (it.netout ? it.netout : 0)),
            ],
            color: this.colors,
            func: this.byteToSize,
            time: res.data.map((it) => it.time),
          ...
          this.disk = Object.assign({}, this.disk, {
            value: [
              res.data.map((it) => (it.diskread ? it.diskread : 0)),
              res.data.map((it) => (it.diskwrite ? it.diskwrite : 0)),
            ],
            color: this.colors,
            func: this.byteToSize,
            time: res.data.map((it) => it.time),
    ...
    setComment() {
      ...
      return this.$http
        .put(
          `/json/nodes/${this.node.node}/${this.node.id}/config`,
          {
