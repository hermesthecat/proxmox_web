...
export default {
  ...
  methods: {
    ...
    createBackUp(param) {
      ...
      return this.$http
        .post(`json/nodes/${this.node.node}/vzdump`, param, {
          headers: {
    ...
    delete() {
      let event = this.createEvent(`action.qemu.backup.delete`),
        tasks = [],
        p = null;
      this.selectedList.forEach((item) => {
        ((id) => {
          p = this.$http
            .del(
              `json/nodes/${this.node.node}/storage/${this.storage}/content/${id}`,
              {
                delay: 5,
              }
            )
    ...
    queryConfig(param) {
      return this.$http
        .get(`json/nodes/${this.node.node}/vzdump/extractconfig`, param)
        .then((res) => {
          if (res.data) {
