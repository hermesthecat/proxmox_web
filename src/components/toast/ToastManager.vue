...
export default {
  ...
  methods: {
    ...
    getClass(item) {
      let obj = {
        running: item.successCount + item.failCount < item.count,
        success:
          item.successCount === item.count || item.resultType === "success",
        fail: item.failCount === item.count || item.resultType === "fail",
        other:
          item.successCount + item.failCount >= item.count &&
          item.failCount !== item.count &&
...
.toast.success {
  border: 1px solid rgba(69, 187, 121, 0.3);
